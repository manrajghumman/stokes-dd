...
namespace dd_stokes
{
  ...
  { 
    ...
    tensor_value [0][0] = -std::cos(p[0]*p[1])+2*std::cos(p[0]);
    tensor_value [0][1] = 0;
    tensor_value [1][0] = 0;
    tensor_value [1][1] = -std::cos(p[0]*p[1])+2*std::cos(p[1]);
  ...
  Tensor<1, dim> RightHandSide<dim>::value(const Point<dim> & p) const
  {
    ...
    // Compute the right-hand side values based on the given point 'p'
        ...
        // value = f(x) at x = 'p'
        rhs_value[0] = 2*std::sin (p[0])-p[1]*std::sin (p[0]*p[1]);
        rhs_value[1] = 2*std::sin (p[1])-p[0]*std::sin (p[0]*p[1]);
...
template <int dim>
  ...
  template <int dim>
                                          ...
                                          Vector<double> &  values) const
    {
      ...
      values(0) = std::sin(p[0]);
      values(1) = std::sin(p[1]);
      values(2) = std::cos (p[0]*p[1]);
    }
  ...
  template <int dim>
    ...
    {
      gradients[0][0] = std::cos(p[0]);
      gradients[0][1] = 0;
      gradients[1][0] = 0;
      gradients[1][1] = std::cos(p[1]);
      gradients[2][0] = -p[1]*std::sin(p[0]*p[1]);
      gradients[2][1] = -p[0]*std::sin(p[0]*p[1]);
    }
