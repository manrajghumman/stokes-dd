...
namespace Projector
{
  ...
  // Default constructor for CopyData class initializes 
  // dofs_per_cell with numbers::invalid_unsigned_int
  template <typename DoFHandlerType, typename number>
  CopyData<DoFHandlerType, number>::CopyData()
    : dofs_per_cell(numbers::invalid_unsigned_int)
  {}

  // Constructor for CopyData class
  /*
  This constructor is used to deep copy the data from one CopyData
  object to another. It initializes the member variables
  with the values from the input CopyData object.
  It is used in the context of the WorkStream class to
  transfer data between threads.
  The CopyData class is used to store data that is needed
  for the computation of the boundary mass matrix.
  */
  template <typename DoFHandlerType, typename number>
